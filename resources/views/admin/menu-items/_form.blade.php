...
@if (!empty($locales))
    <div class="mt-4">
            ...
            @foreach ($locales as $locale)
                <li class="nav-item" role="presentation">
                    <button class="nav-link @if ($loop->first) active @endif" data-bs-toggle="tab"
                        data-bs-target="#mi-{{ $locale }}" type="button" role="tab">
                        {{ strtoupper($locale) }}
                    </button>
                </li>
            @endforeach
        ...
        <div class="tab-content border border-top-0 p-3">
            @foreach ($locales as $locale)
                @php $t = $item->translation($locale); @endphp
                <div class="tab-pane fade @if ($loop->first) show active @endif" id="mi-{{ $locale }}" role="tabpanel">
