             ...
             <div class="slide {{ $index !== 0 ? 'lazy-background' : '' }}"
                 ...
                 @if ($index === 0)
                     <img src="{{ $movie['webp_image_url'] }}" alt="{{ $movie['title'] ?? $movie['name'] }}"
                         class="d-block w-100" width="1280" height="720"
                         fetchpriority="{{ $index === 0 ? 'high' : 'auto' }}" decoding="auto"
                         loading="{{ $index === 0 ? 'eager' : 'lazy' }}" />
                 @endif

                 <div class="auto-container custom-height">
                     @if ($index === 0)
                         <div class="content-boxed no-anim">
                         @else
                             <div class="content-boxed animate__fadeIn">
                     @endif
