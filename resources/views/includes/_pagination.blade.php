...
@if($paginator && method_exists($paginator, 'lastPage') && $paginator->hasPages())
  ...
  <div class="pg" dir="{{ $rtl ? 'rtl' : 'ltr' }}">
    <nav class="pg__nav" role="navigation" aria-label="{{ __('document_ui.movies.pagination_aria') }}">
      @if ($paginator->onFirstPage())
        <span class="pg__btn" aria-disabled="true" aria-label="{{ __('document_ui.movies.previous_page_aria') }}">
          @if($rtl)
          ...
          @endif
          {{ __('document_ui.movies.previous_label') }}
        </span>
      @else
        <a class="pg__btn" href="{{ $paginator->previousPageUrl() }}" rel="prev" aria-label="{{ __('document_ui.movies.previous_page_aria') }}">
          @if($rtl)
            <svg class="pg__icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9 6l6 6-6 6" stroke="#111" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
          @else
            <svg class="pg__icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M15 6l-6 6 6 6" stroke="#111" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
      ...
      <ul class="pg__list">
        @foreach ($pages as $page)
          ...
          @else
            <li><a class="pg__page" href="{{ $paginator->url($page) }}"
                aria-label="{{ __('document_ui.movies.page_aria', ['page' => $page]) }}">{{ $page }}</a></li>
          @endif
        @endforeach
      </ul>

      @if ($paginator->hasMorePages())
        <a class="pg__btn" href="{{ $paginator->nextPageUrl() }}" rel="next" aria-label="{{ __('document_ui.movies.next_page_aria') }}">
          {{ __('document_ui.movies.next_label') }}
          @if($rtl)
            <svg class="pg__icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M15 6l-6 6 6 6" stroke="#111" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
          @else
            <svg class="pg__icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9 6l6 6-6 6" stroke="#111" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
      ...
      @else
        <span class="pg__btn" aria-disabled="true" aria-label="{{ __('document_ui.movies.next_page_aria') }}">
          {{ __('document_ui.movies.next_label') }}
          @if($rtl)
            <svg class="pg__icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M15 6l-6 6 6 6" stroke="#111" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
          @else
    ...
    <div class="pg__stats">
      {{ __('document_ui.movies.pagination_summary', [
