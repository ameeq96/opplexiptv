...
@php
    ...
    $shareLabel = __('interface.blog.copy_link');
...
@section('content')
    ...
    <section class="blogs-wrap blog-single {{ $isRtl ? 'rtl' : '' }}">
        <div class="auto-container">
            ...
            <nav class="blog-breadcrumbs" aria-label="{{ __('interface.blog.breadcrumb_aria') }}">
            ...
            <article class="blog-article">
                <header class="blog-article__header">
                    ...
                    <div class="blog-article__meta">
                        ...
                        @if ($blog->views)
                            <span class="blog-meta__sep" aria-hidden="true"></span>
                            <span class="blog-meta__item" aria-label="{{ __('interface.blog.views', ['count' => number_format($blog->views)]) }}">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                    stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                    <path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7-10-7-10-7z" />
                                    <circle cx="12" cy="12" r="3" />
                                </svg>
                                {{ __('interface.blog.views', ['count' => number_format($blog->views)]) }}
                            </span>
                        @endif
                    </div>
                </header>
                ...
                <div class="blog-article__layout">
                    <aside class="blog-share-rail" aria-label="{{ $shareLabel }}">
                        <span class="blog-share-rail__label">{{ __('interface.blog.share') }}</span>
                        <a class="blog-share-btn" data-net="x" href="{{ $shareLinks['x'] }}" target="_blank"
                            rel="noopener nofollow" aria-label="{{ __('interface.blog.share_on', ['network' => 'X']) }}">
                        ...
                        <a class="blog-share-btn" data-net="fb" href="{{ $shareLinks['fb'] }}" target="_blank"
                            rel="noopener nofollow" aria-label="{{ __('interface.blog.share_on', ['network' => 'Facebook']) }}">
                        ...
                        <a class="blog-share-btn" data-net="in" href="{{ $shareLinks['in'] }}" target="_blank"
                            rel="noopener nofollow" aria-label="{{ __('interface.blog.share_on', ['network' => 'LinkedIn']) }}">
                        ...
                        <a class="blog-share-btn" data-net="wa" href="{{ $shareLinks['wa'] }}" target="_blank"
                            rel="noopener nofollow" aria-label="{{ __('interface.blog.share_on', ['network' => 'WhatsApp']) }}">
                        ...
                        <button class="blog-share-btn blog-share-btn--copy" type="button" id="blogCopyBtn"
                            aria-label="{{ $shareLabel }}" data-copied-label="{{ __('interface.blog.copied') }}">
                ...
                @if ($authorName)
                    <div class="blog-author-card">
                        ...
                        <div class="blog-author-card__body">
                            <span class="blog-author-card__role">{{ __('interface.blog.written_by') }}</span>
                ...
                <div class="blog-article__footer">
                    ...
                    <div class="blog-share">
                        <button type="button" id="blogFooterCopyBtn"
                            data-copied-label="{{ __('interface.blog.copied') }}">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                <path d="M10 13a5 5 0 0 0 7 0l3-3a5 5 0 0 0-7-7l-1 1" />
                                <path d="M14 11a5 5 0 0 0-7 0l-3 3a5 5 0 0 0 7 7l1-1" />
...
@section('script')
    <script>
        (function () {
            ...
            if (copyBtn) {
                copyBtn.addEventListener('click', function () {
                    navigator.clipboard.writeText(window.location.href).then(function () {
                        ...
                        copyBtn.setAttribute('aria-label', copyBtn.dataset.copiedLabel);
                        setTimeout(function () {
            ...
            var footerCopyBtn = document.getElementById('blogFooterCopyBtn');
            if (footerCopyBtn) {
                footerCopyBtn.addEventListener('click', function () {
                    navigator.clipboard.writeText(window.location.href).then(function () {
                        var output = footerCopyBtn.querySelector('.js-copy-text');
                        if (!output) return;
                        var originalLabel = output.dataset.label;
                        output.textContent = footerCopyBtn.dataset.copiedLabel;
                        footerCopyBtn.classList.add('is-copied');
                        setTimeout(function () {
                            output.textContent = originalLabel;
                            footerCopyBtn.classList.remove('is-copied');
                        }, 1800);
                    });
                });
            }
